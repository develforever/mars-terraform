    ...
    <group position={[wx, 0, wz]}>
      <group ref={groupRef} position={[0, 1.5, 0]}>
        <mesh
          onClick={e => {
            ...
            else if (activeTool === 'erase') removeResourceNode(node.id)
          }}
        >
      ...
      <Html position={[0, 3, 0]} center distanceFactor={60} occlude={false}>
        <div
          ...
          style={{ backgroundColor: isSelected ? color + 'dd' : '#18181bcc' }}
        >
...
const ResourceNodeMarkers = () => {
  ...
  return (
    <>
      {resourceNodes.map(n => <ResourceMarker key={n.id} nodeId={n.id} />)}
    </>
  )
}
